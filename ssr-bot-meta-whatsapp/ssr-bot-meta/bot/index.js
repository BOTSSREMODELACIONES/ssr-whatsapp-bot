...
async function handleMessage(from, text, messageId) {
  ...
  try {
    ...
    if (flag === "ESCALAR") {
      ...
      const [name, project, zone, dayPref] = (flagData || "").split("|");
      const updated = update(from, {
        name: name?.trim() || session.name,
        project_desc: project?.trim() || session.project_desc,
        zone: zone?.trim() || session.zone,
        visit_date: dayPref?.trim() || "a coordinar",
...
async function notifyMelvin(from, session, lastMsg, tipo) {
  const header = {
    visita_solicitada: "🗓️ NUEVA VISITA SOLICITADA",
  ...
  const lines = [
    ...
    session.visit_date   && `📅 Preferencia: ${session.visit_date}`,
    "", `💬 "${lastMsg}"`, "",
    "_Sasha — Bot SSR_",
  ].filter(Boolean).join("\n");
